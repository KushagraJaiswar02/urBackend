...
export class DatabaseModule {
  ...
  /**
   * Explicitly auto-create a collection configuration in the backend
   */
  public async createCollection(name: string): Promise<{ name: string; created: boolean }> {
    return this.client.request<{ name: string; created: boolean }>('POST', '/api/collections', {
      body: { name }
    });
  }
